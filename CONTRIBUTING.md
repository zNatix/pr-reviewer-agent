...
4. **Recommended model for testing**: `claude-sonnet-4-6` (catches more edge cases in instructions). Switch back to `gpt-5.2-codex` for production to control costs
...
3. Set `model` as an array with at least one fallback (e.g. `["gpt-5.2-codex", "claude-sonnet-4-6"]`)
...
- [ ] `excludeAgent` is an array, not a string
