...
> ⚠️ **Trusted branch only**: This agent may run read-only validation commands (`dotnet test --list-tests`, build inspections, etc.) when explicitly requested. Never run arbitrary commands on untrusted fork PRs.
...
- NEVER run build, test, package restore, scripts, source generators, project tooling, or arbitrary commands on untrusted fork PRs
